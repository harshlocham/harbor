...
| Package             | Role                                                             |
| ------------------- | ---------------------------------------------------------------- |
| `@harborts/core`      | `Agent`, `Runtime`, tools, messages, provider interfaces, errors |
| `@harborts/providers` | `OpenAIProvider` and OpenAI message/tool mappers                 |
| `@harborts/utils`     | Shared utilities (stub today)                                    |
...
flowchart TB
  ...
  subgraph corePkg ["@harborts/core"]
    agent
    runtime
    mp
  end
  subgraph providersPkg ["@harborts/providers"]
    openai
  end
```
...
`Session` is a public **type** only — Harbor does not persist sessions. `@harborts/utils` is a stub.
