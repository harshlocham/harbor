...
Harbor talks to models through the `ModelProvider` interface in `@harbor/core`. Vendor SDKs live only in `@harbor/providers` (or your own adapter package).
...
```text
@harbor/providers ──depends on──▶ @harbor/core
@harbor/core      ──must not──▶  @harbor/providers
```
...
// or: import { OpenAIProvider } from "@harbor/providers/openai";
...
```ts
import { Agent, MockProvider, Runtime } from "@harbor/core";
